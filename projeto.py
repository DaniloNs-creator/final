...
# Função para carregar dados de forma controlada
...
def main():
    ...
    if 'mes_ano_referencia' not in st.session_state:
        ...
        load_and_set_df(st.session_state.mes_ano_referencia) # Carrega no início
    ...
    # Garantir que st.session_state.df_atividades está sempre inicializado e correto para o mês atual
    # Este bloco é importante para re-sincronizar se o estado for perdido ou a página recarregada
    if 'df_atividades' not in st.session_state or \
       (not st.session_state.df_atividades.empty and st.session_state.df_atividades['MesAnoReferencia'].iloc[0] != st.session_state.mes_ano_referencia) or \
       (st.session_state.df_atividades.empty and not st.session_state.mes_ano_referencia.startswith('')) : # Pequeno truque para garantir recarga inicial se estiver vazio
        load_and_set_df(st.session_state.mes_ano_referencia)
    ...
    if st.session_state.df_atividades.empty:
        ...
        if st.button(f"Habilitar Mês {st.session_state.mes_ano_referencia}", key="habilitar_mes_btn"):
            ...
            for activity in initial_templates:
                ...
                activity['Data Início'] = None # Assegurar que Data Início seja None no início
                activity['Data Conclusão'] = None # Assegurar que Data Conclusao seja None no início
                activities_to_insert.append(activity)
            
            insert_initial_data(activities_to_insert)
            load_and_set_df(st.session_state.mes_ano_referencia) # Recarrega o DF após inserção
            st.success(f"Atividades padrão para {st.session_state.mes_ano_referencia} habilitadas com sucesso!")
            st.rerun() # Use st.rerun() para atualizar a UI imediatamente
    ...
    if not st.session_state.df_atividades.empty:
        ...
        with st.expander("🔍 Filtros", expanded=True):
            ...
            with col1:
                ...
                # Apenas popula o selectbox com opções únicas se a coluna existir e o DF não for vazio
                current_status_filter = status_options[0] # Default
                if 'Status' in filtered_df.columns and not filtered_df.empty:
                    ...
                    st.selectbox("Status", status_options, disabled=True, key="filter_status_disabled")

            with col2:
                difficulty_options = ["Todos", "Baixa", "Média", "Alta"]
                current_difficulty_filter = difficulty_options[0] # Default
                if 'Dificuldade' in filtered_df.columns and not filtered_df.empty:
                    current_difficulty_filter = st.selectbox("Dificuldade", difficulty_options, key="filter_difficulty")
                else:
                    st.selectbox("Dificuldade", difficulty_options, disabled=True, key="filter_difficulty_disabled")
            with col3:
                orgao_options = ["Todos"]
                current_orgao_filter = orgao_options[0] # Default
                if 'Órgão Responsável' in filtered_df.columns and not filtered_df.empty:
                    ...
                    st.selectbox("Órgão Responsável", orgao_options, disabled=True, key="filter_orgao_disabled")
        ...
        if not filtered_df.empty:
            ...
            if current_orgao_filter != "Todos":
                filtered_df = filtered_df[filtered_df['Órgão Responsável'] == current_orgao_filter]
        ...
        with st.form("nova_atividade_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Adicionar Atividade"):
                if nova_obrigacao and nova_descricao and novo_orgao and nova_data_limite and novo_prazo:
                    ...
                    load_and_set_df(st.session_state.mes_ano_referencia) # Recarrega o DF após adição
        ...
        if not st.session_state.df_atividades.empty:
            ...
            if atividade_selecionada:
                ...
                if st.button("Atualizar Atividade Selecionada", key="update_activity_btn"):
                    # Sinalizador para saber se alguma atualização ocorreu
                    ...
                    if updated_something:
                        load_and_set_df(st.session_state.mes_ano_referencia) # Recarrega o DF após atualização
                        st.rerun() # Use st.rerun()
                    else:
    ...
    if todas_finalizadas_ou_fechadas and not st.session_state.df_atividades.empty:
        ...
        if st.button("Fechar Período e Habilitar Próximo Mês", key="fechar_periodo_btn"):
            ...
            st.session_state.mes_ano_referencia = get_next_month_year(st.session_state.mes_ano_referencia)
            load_and_set_df(st.session_state.mes_ano_referencia) # Recarrega o DF para o novo mês
