...
# ==============================================================================
# 1. PARSER APP 2 (HÄFELE) - ENGINE AVANÇADA (REGEX + BLOCKS)
# ==============================================================================

class HafelePDFParser:
    """
    Parser avançado para PDFs da Häfele.
    Utiliza segmentação por blocos de texto e Regex tolerante a falhas de OCR (ex: '11' vs 'II').
    """
    ...
    def parse_pdf(self, pdf_path: str) -> Dict:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for page in pdf.pages:
                # layout=True é essencial para manter a 'visibilidade' da tabela
    ...
    def _process_full_text(self, text: str):
        ...
        items = []
        if len(chunks) > 1:
            # chunks[1]=NumeroItem, chunks[2]=ConteudoItem...
    ...
    def _parse_item_content(self, text: str, item_num: int) -> Dict:
        # Normaliza texto para facilitar buscas (remove acentos apenas para busca de chaves, não valores)
        ...
        # 1. CÓDIGO INTERNO
        # Regex busca: "Codigo interno" ... (numero)
        code_match = re.search(r'Codigo interno[^\d]*([\d\.]+)', text_norm, re.IGNORECASE)
        if code_match:
            item['codigo_interno'] = code_match.group(1).strip()

        # 2. DESCRIÇÃO COMPLEMENTAR (Para concatenação)
        # Busca no texto original (com acentos) para manter a qualidade da string
        desc_match = re.search(r'DESCRIÇÃO COMPLEMENTAR DA MERCADORIA\s*\n?(.*?)(?=\n|CONDIÇÃO|NCM)', text, re.IGNORECASE | re.DOTALL)
        if desc_match:
            item['descricao_complementar'] = desc_match.group(1).strip().replace('\n', ' ')

        # 3. VALORES LOGÍSTICOS
        # Usamos regex que captura formato brasileiro 1.000,00
        item['frete_internacional'] = self._extract_value_regex(r'Frete Internac', text_norm)
        item['seguro_internacional'] = self._extract_value_regex(r'Seguro Internac', text_norm)
        item['local_aduaneiro'] = self._extract_value_regex(r'Local Aduaneiro', text_norm)

        # 4. IMPOSTOS - ENGINE DE BLOCOS
        # Localiza os cabeçalhos das seções de impostos
        # "11" é comum no lugar de "II" devido ao OCR/Fonte do PDF
        idx_ii = -1
        match_ii = re.search(r'\b(II|11)\b', text_norm) 
        if match_ii: idx_ii = match_ii.start()
        
        match_ipi = re.search(r'\bIPI\b', text_norm)
        idx_ipi = match_ipi.start() if match_ipi else -1
        
        match_pis = re.search(r'\bPIS\b', text_norm)
        idx_pis = match_pis.start() if match_pis else -1
        
        match_cofins = re.search(r'\bCOFINS\b', text_norm)
        idx_cofins = match_cofins.start() if match_cofins else -1

        # Recorta o texto para cada imposto
        def get_block(start, ends):
            if start == -1: return ""
            valid_ends = [x for x in ends if x > start]
            end = min(valid_ends) if valid_ends else len(text_norm)
        ...
        block_cofins = text_norm[idx_cofins:] if idx_cofins != -1 else ""

        # Extrai dados de cada bloco
    ...
    def _populate_tax(self, item: Dict, prefix: str, block: str):
        ...
        # Regex flexível para chaves (sem acento)
        # Valor Devido
        val = self._extract_value_regex(r'Valor Devido', block)
        if val == 0: val = self._extract_value_regex(r'Valor Calculado', block)
        if val == 0: val = self._extract_value_regex(r'Valor A Recolher', block)
        
        # Base de Cálculo
        base = self._extract_value_regex(r'Base de Calculo', block)
        
        # Alíquota (Pode ser "% Alíquota" ou "Ad Valorem")
        aliq = self._extract_value_regex(r'% Aliquota', block)
        if aliq == 0: aliq = self._extract_value_regex(r'Ad Valorem', block)

        item[f'{prefix}_valor_devido'] = val
        item[f'{prefix}_base_calculo'] = base
        item[f'{prefix}_aliquota'] = aliq

    def _extract_value_regex(self, key_pattern: str, text: str) -> float:
        """
        Busca um padrão monetário brasileiro (X.XXX,XX ou X,XX) que venha DEPOIS da chave.
        Ignora caracteres intermediários não-numéricos.
        """
        try:
            # Pattern: Chave + (lixo) + (Numero com ponto opcional e virgula obrigatoria + decimais)
            # Ex: "Valor Devido (R$)\n . . . 1.234,56"
            regex = key_pattern + r'[^\d]*([\d\.]*,\d+)'
    ...
    def _normalize_text(self, text: str) -> str:
        """Remove acentos e força maiúsculas para facilitar regex"""
                    ...
                    for idx, row in df_dest.iterrows():
                        try:
                            ...
                            if item_num in src_map:
                                ...
                                # CONCATENAÇÃO SOLICITADA: CÓDIGO + DESCRIÇÃO COMPLEMENTAR
