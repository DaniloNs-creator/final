...
# CONFIGURAÇÃO GERAL
# ==============================================================================
st.set_page_config(page_title="Sistema Integrado DUIMP 2026 (Ultimate)", layout="wide")
...
# ==============================================================================
# 1. PARSER APP 2 (HÄFELE) - VERSÃO DEFINITIVA (MULTIPAGE + ROBUST REGEX)
# ==============================================================================
...
class HafelePDFParser:
    ...
    def parse_pdf(self, pdf_path: str) -> Dict:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # Junta todo o texto para tratar itens que quebram de página
            all_text_content = []
            for page in pdf.pages:
                # layout=True mantém a posição visual, essencial para tabelas
                text = page.extract_text(layout=True)
                if text:
                    # Remove cabeçalhos de página repetitivos para limpar o fluxo
                    # Ex: "--- PAGE X ---" ou cabeçalhos fixos se houver
                    clean_page = re.sub(r'--- PAGE \d+ ---', '', text)
                    all_text_content.append(clean_page)
    ...
    def _process_full_text(self, text: str):
        ...
        # Usamos re.split para quebrar o texto gigante em blocos de itens
        chunks = re.split(r"ITENS DA DUIMP-(\d+)", text)
        
        items = []
        if len(chunks) > 1:
            # chunks[0] = lixo inicial
            # chunks[1] = Numero Item 1 ("00001")
            # chunks[2] = Conteudo Item 1
            # chunks[3] = Numero Item 2...
            for i in range(1, len(chunks), 2):
                try:
                    ...
                    # Processa o bloco de texto deste item específico
    ...
    def _parse_item_content(self, text: str, item_num: int) -> Dict:
        ...
        # 1. CÓDIGO INTERNO
        # Procura "Código interno" seguido de quebras/espaços e captura números/pontos
        ...
        # Regex Robusta: Label + (lixo não numérico) + Valor
        ...
        # 4. IMPOSTOS (Lógica de Segmentação de Texto)
        ...
        # PIS pode confundir com outros textos, buscamos contexto
