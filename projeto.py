...
st.set_page_config(page_title="Conversor DUIMP V14 (Cálculo IBS/CBS)", layout="wide")
...
ADICAO_FIELDS_ORDER = [
    ...
    {"tag": "cofinsAliquotaAdValorem", "default": "00000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "cofinsAliquotaValorDevido", "default": "000000000000000"}, # PREENCHIDO DO PDF
    {"tag": "cofinsAliquotaValorRecolher", "default": "000000000000000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "iiAliquotaAdValorem", "default": "00000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "iiAliquotaValorDevido", "default": "000000000000000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "ipiAliquotaAdValorem", "default": "00000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "ipiAliquotaValorDevido", "default": "000000000000000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "pisPasepAliquotaAdValorem", "default": "00000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "pisPasepAliquotaValorDevido", "default": "000000000000000"}, # PREENCHIDO DO PDF
    ...
    {"tag": "cbsBaseCalculoValorImposto", "default": "00000000000000"}, # CALCULADO (0.90%)
    ...
    {"tag": "ibsBaseCalculoValorImposto", "default": "00000000000000"}, # CALCULADO (0.10%)
...
class DataFormatter:
    ...
    def calculate_cbs_ibs(base_xml_string):
        """
        Calcula IBS e CBS baseado na alíquota 2026 sobre a Base Formatada XML.
        CBS = Base * 0.9% (0.009)
        IBS = Base * 0.1% (0.001)
        """
        try:
            # Base vem como string "000000000100000" -> 1000.00
            base_int = int(base_xml_string)
            base_float = base_int / 100.0
            
            cbs_val = base_float * 0.009
            # Formata de volta para 14 ou 15 dígitos sem ponto
...
# ==============================================================================
# 3. PARSER V14 (STITCHER + TAX SCANNER + 74 ITEMS)
# ==============================================================================
...
class PDFParserV14:
    ...
    def preprocess(self):
        """
        Une todas as 450 páginas em um texto contínuo, 
        removendo cabeçalhos que quebram itens.
        """
        raw_text_parts = []
        ...
        for i, page in enumerate(self.doc):
            ...
            for line in lines:
                ...
                if not is_garbage:
                    # Injeta marcador se encontrar início de item
                    if re.search(r"Nº\s*Adição\s*[:\n]?\s*\d+", line, re.IGNORECASE):
                        clean_lines.append(ITEM_MARKER)
    ...
    def _extract_tax_robust(self, tax_label, block_text):
        """
        Caça alíquota e valor do imposto.
        """
        ...
        if len(nums) >= 2:
            ...
            if candidates:
                ...
                # Menor = Alíquota, Maior/Médio = Valor
                rate = candidates[0][1]
    ...
    def extract_items(self):
        chunks = self.full_text.split("<<<<ITEM_START>>>>")
        
        if len(chunks) > 1:
            st.info(f"Processando {len(chunks)-1} itens...")
            ...
            for block in chunks[1:]:
                ...
                raw_desc = desc_match.group(1).strip() if desc_match else f"ITEM {item['numeroAdicao']}"

                if part_number:
                    item["descricao"] = f"{part_number} - {raw_desc}"
                else:
                    item["descricao"] = raw_desc
            ...
            st.error("Erro: Marcadores de item não encontrados.")
...
class XMLBuilder:
    ...
    def build(self):
        ...
        for it in self.p.items:
            ...
            # Base = Valor Mercadoria (Total)
            # CBS = Base * 0.9%
            # IBS = Base * 0.1%
            cbs_imposto, ibs_imposto = DataFormatter.calculate_cbs_ibs(base_total_reais)
            ...
            extracted_map = {
                ...
                # --- IMPOSTOS CALCULADOS (IBS/CBS) ---
                "icmsBaseCalculoValor": base_total_reais,
                "icmsBaseCalculoAliquota": "01800",
                "cbsIbsClasstrib": "000001",
                "cbsBaseCalculoValor": base_total_reais,
                "cbsBaseCalculoAliquota": "00090", # 0.90%
                "cbsBaseCalculoValorImposto": cbs_imposto,
                "ibsBaseCalculoValor": base_total_reais,
...
st.title("Conversor DUIMP V14 (Cálculo IBS/CBS + Scan Fiscal)")
st.markdown("Extração completa com cálculo automático de IBS/CBS 2026.")
    ...
    if st.button("Gerar XML"):
        try:
            with st.spinner("Processando..."):
                ...
                st.success(f"Concluído! {len(p.items)} itens processados.")
