        ...
        with get_db_connection() as conn:
            ...
            c.execute('''
                INSERT INTO atividades (
                    ...
                    grupo, cidade, desde, status, email, telefone, contato, possui_folha, 
                    financeiro, quadro_contas_bancarias, forma_entrega, empresa_administrada, protesta_no_bancario, 
                    parcela_perto, catrato, saldo_anterior, extrato, mes_referencia, data_criacao
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', campos_completos)
            conn.commit()
...
def adicionar_atividades_em_lote(dados: List[Tuple]) -> bool:
    ...
    try:
        with get_db_connection() as conn:
            ...
            # Preparar os dados com data de criação e status padrão
            dados_completos = [
                (*linha, datetime.now().strftime('%Y-%m-%d %H:%M:%S'), 0) 
                for linha in dados
            ]
            ...
            try:
                c.executemany('''
                    INSERT INTO atividades (
                        ...
                        grupo, cidade, desde, status, email, telefone, contato, possui_folha, 
                        financeiro, quadro_contas_bancarias, forma_entrega, empresa_administrada, protesta_no_bancario, 
                        parcela_perto, catrato, saldo_anterior, extrato, mes_referencia, data_criacao, feito
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', dados_completos)
