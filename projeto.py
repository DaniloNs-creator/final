...
class HafelePDFParser:
    ...
    def _parse_item(self, text: str, item_num: str, ncm: str, codigo: str) -> Optional[Dict]:
        ...
        try:
            item = {
                ...
                'valor_unitario': 0,
                'valor_total': 0,
                'valor_aduaneiro': 0, # Alterado de local_aduaneiro para valor_aduaneiro
            ...
            # === LÓGICA DE VALOR ADUANEIRO ===
            # Prioridade 1: Local Aduaneiro
            vlr_adu_match = re.search(r'Local Aduaneiro \(R\$\)\s*([\d\.,]+)', text, re.IGNORECASE)
            if vlr_adu_match:
                item['valor_aduaneiro'] = self._parse_valor(vlr_adu_match.group(1))
            else:
                # Prioridade 2: Fallback para Base de Cálculo (geralmente a primeira base que aparece é o VA)
                base_match = re.search(r'Base de Cálculo \(R\$\)\s*([\d\.,]+)', text, re.IGNORECASE)
                if base_match:
                    item['valor_aduaneiro'] = self._parse_valor(base_match.group(1))
            ...
            # Se valor aduaneiro não foi capturado, tenta estimar pela soma
            if item['valor_aduaneiro'] == 0 and item['valor_total'] > 0:
                 # Fallback simples caso necessário
                 item['valor_aduaneiro'] = item['valor_total'] * 5.5 # Exemplo de taxa, mas melhor deixar 0 se não achar
            
            item['valor_total_com_impostos'] = item['valor_aduaneiro'] + item['total_impostos']
    ...
    def _extract_taxes_directly(self, text: str, item: Dict) -> Dict:
        ...
        # Mapeamento para Base de Cálculo
        base_values = {
    ...
    def _calculate_totals(self):
        ...
        for item in self.documento['itens']:
            ...
            totais['valor_aduaneiro_total'] += item.get('valor_aduaneiro', 0)
    ...
    def prepare_dataframe(self):
        ...
        for item in self.documento['itens']:
            itens_data.append({
                ...
                # COLUNA SOLICITADA
                'Valor Aduaneiro (R$)': item.get('valor_aduaneiro', 0),
...
def main():
    ...
    <div class="section-card">
        ...
        Sistema ajustado para ler Códigos Internos (Limpos), Valor Aduaneiro, Bases de Cálculo e Alíquotas.
    </div>
    """, unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        try:
            ...
            with col1:
                st.markdown(f"""
                <div class="section-card">
                    <div class="metric-value">R$ {totais.get('valor_aduaneiro_total', 0):,.2f}</div>
                    <div class="metric-label">Valor Aduaneiro Total</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            # Ordenação das colunas para visualização lógica (INCLUI VALOR ADUANEIRO)
            cols_order = [
                'Item', 'Código Interno', 'Produto', 'NCM',
                'Valor Total (R$)', 'Valor Aduaneiro (R$)', 
