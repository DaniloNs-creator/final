...
def initialize_session_state():
    """Inicializa todas as variáveis do session_state de forma segura usando notação de dicionário"""
    session_vars = {
        'batch_size': 100,
...
class CTeProcessorOptimized:
    def __init__(self):
        self.processed_data = []
        # Garantir que processed_files está inicializado usando notação de dicionário
    ...
    def process_multiple_files_optimized(self, uploaded_files):
        ...
        # Usar notação de dicionário consistentemente
...
def processador_cte():
    ...
    # Inicializar processador no session_state usando notação de dicionário
    ...
    with tab3:
        ...
        if not df.empty:
            ...
            with col1:
                st.metric("Total de CT-es", len(df))
            with col2:
                st.metric("Valor Total", f"R$ {df['Valor Prestação'].sum():,.2f}")
            with col3:
                st.metric("Peso Total", f"{df['Peso Bruto (kg)'].sum():,.0f} kg")
            ...
            st.info("📝 Nenhum dado processado. Faça o upload de arquivos XML na aba 'Upload'.")
...
def main():
    ...
    <div style='text-align: center; color: #7f8c8d; font-size: 0.9rem;'>
        💡 <strong>Dica:</strong> Use a notação de dicionário (st.session_state["chave"]) para acessar variáveis de sessão de forma segura.
    </div>
    """, unsafe_allow_html=True)
